...
class PeminjamController extends Controller
{
    ...
    public function delete ($id_peminjam){
        $query = \DB::table('peminjam')->where('id_peminjam',$id_peminjam)->delete();
        return redirect('peminjam');
    }

    public function edit($id_peminjam){
        $data['peminjam'] = Peminjam_model::where('id_peminjam',$id_peminjam)->first();
        return view('peminjam/edit', $data);
    }

    public function edit_proses(Request $request){
        // return ('Edit Proses');
        $query = Peminjam_model::where('id_peminjam',$request->id_peminjam)
        ->update([
            'nim_peminjam' => $request->nim_peminjam,
            'nama_peminjam' => $request->nama_peminjam,
            'alamat' => $request->alamat,
